...
**Inventory Manager** is a desktop app for small businesses that **buy stock from suppliers, sell it to customers, and often sell on credit.**
...
**Backup & restore**
- **Create backup…** — choose where to save. The suggested name is `pos-backup-YYYY-MM-DD.db`. The file contains **all** your data. Afterwards you'll see the file name, time and size.
...
- **Everything is in one database file** on this computer. On macOS it's here:
  `~/Library/Application Support/inventory-manager/inventory.db`
...
*This guide describes the "Phase 4" build of Inventory Manager.*
