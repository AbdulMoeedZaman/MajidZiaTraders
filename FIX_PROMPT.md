...
# Task: fix every issue found in the MZTraders deep test

You're working on **MZTraders**, an offline desktop app for inventory, invoicing and customer accounts. Fix **all** issues listed below, verify each fix, and report back.
...
- **Fix:**
  - Migration 004 inserts a default profile row if none exists (name `MZTraders`, currency `PKR`, prefix `INV-`, next number 1, tax 0).
